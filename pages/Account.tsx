...
export default function Account() {
  ...
  const userKey = effectiveUser?.id || 'no-user';
  
  // Handle user state changes
  useEffect(() => {
    if (effectiveUser && effectiveUser.role === 'vendor') {
      // Load vendor data when user becomes available
  ...
  useEffect(() => {
    ...
    const checkAuthState = async () => {
      try {
        ...
        if (session?.user) {
          ...
          console.log('Account: initial auth check found user:', profile);
          setFallbackUser(profile);
        } else {
          // No user found, redirect to landing page
          console.log('Account: no user found, redirecting to landing page');
          router.push('/');
        }
        setAuthChecked(true);
      } catch (error) {
    ...
    const { data: { subscription } } = supabase.auth.onAuthStateChange((event, session) => {
      ...
      if (event === 'SIGNED_OUT') {
        console.log('Account: user signed out, redirecting to landing page');
        setFallbackUser(null);
        // Redirect to landing page immediately on sign out
        router.push('/');
      }
    });
  ...
  useEffect(() => {
    const loadData = async () => {
      ...
      // Initialize form with current values immediately
      ...
      try {
        ...
        if (currentUser.role === 'vendor') {
          v = await VendorService.getVendorByUserIdCachedFirst(currentUser.id);
          setVendor(v);
        }
        ...
        if (v) {
          ...
          setIsEditing(currentUser.role === 'viewer');
          setBusinessName('');
          setWhatsappNumber('');
          setWebsiteUrl('');
          setFacebookUrl('');
  ...
  // If no user after auth check, we should have redirected already
  // This is just a safety fallback
  if (authChecked && !effectiveUser) {
    return (
      <div className="min-h-screen bg-gray-50 dark:bg-gray-900 flex items-center justify-center transition-colors duration-300">
        <div className="text-center">
          <div className="animate-spin rounded-full h-8 w-8 border-b-2 border-rose-600 mx-auto mb-4"></div>
  ...
  return (
    <div key={userKey} className="min-h-screen bg-gray-50 dark:bg-gray-900 transition-colors duration-300">
          ...
          {isViewer ? (
            ...
            <div className="bg-gray-100 dark:bg-gray-900 rounded-2xl shadow-sm p-6">
                <div className="flex flex-col items-center gap-4 mb-4">
                  ...
                  {isEditing && (
                    <input type="file" accept="image/*" onChange={e => setProfileFile(e.target.files?.[0] || null)} className="block w-full text-sm text-gray-600 dark:text-gray-400" />
                  )}
                </div>
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  ...
                  <div>
                    ...
                    <input value={effectiveUser.email} readOnly className="input-field bg-gray-50 dark:bg-gray-700" />
                  </div>
                </div>
            ...
            <div className="bg-gray-100 dark:bg-gray-900 rounded-2xl shadow-sm overflow-hidden">
              ...
              <div className="p-6 pt-0">
                ...
                <div className="flex items-center justify-between mb-4">
                  <h3 className="text-xl font-semibold text-gray-900 dark:text-gray-100 text-center w-full">{vendor?.business_name || businessName}</h3>
                </div>
                ...
                {!isEditing ? (
                  <div className="space-y-4">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                      ...
                      <div className="bg-gray-50 dark:bg-gray-700 rounded-xl p-4">
                        ...
                        <p className="text-sm font-medium text-gray-900 dark:text-gray-100">{whatsappNumber || '—'}</p>
                      </div>
                      <div className="bg-gray-50 dark:bg-gray-700 rounded-xl p-4">
                        <p className="text-xs text-gray-500">Owner Email</p>
                        <p className="text-sm font-medium text-gray-900 dark:text-gray-100">{effectiveUser.email}</p>
                      </div>
                    </div>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
                      <div className="bg-gray-50 dark:bg-gray-700 rounded-xl p-4">
                        ...
                        <p className="text-sm font-medium text-gray-900 dark:text-gray-100 break-all">{websiteUrl || '—'}</p>
                      </div>
                      <div className="bg-gray-50 dark:bg-gray-700 rounded-xl p-4">
                        <p className="text-xs text-gray-500">Facebook</p>
                        <p className="text-sm font-medium text-gray-900 dark:text-gray-100 break-all">{facebookUrl || '—'}</p>
                      </div>
                      <div className="bg-gray-50 dark:bg-gray-700 rounded-xl p-4">
                        <p className="text-xs text-gray-500">Instagram</p>
                        <p className="text-sm font-medium text-gray-900 dark:text-gray-100 break-all">{instagramUrl || '—'}</p>
                      </div>
                    </div>
                  ...
                  <>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-2">
                      <div>
                        ...
                        <input type="file" accept="image/*" onChange={e => setCoverFile(e.target.files?.[0] || null)} className="block w-full text-sm text-gray-600 dark:text-gray-400" />
                      </div>
                      <div>
                  ...
                  <div>
                    ...
                    <div className="mt-6">
                      <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">Business Types</label>
                        ...
                        {CATEGORIES.filter(c => c !== 'All').map(type => (
                          <label key={type} className="inline-flex items-center gap-2 text-sm text-gray-700 dark:text-gray-300">
                            <input
                              ...
                              className="rounded border-gray-300 dark:border-gray-600 text-rose-700 focus:ring-rose-600"
                            />
                            {type}
                          </label>
                        ))}
                    ...
                    <div className="mt-6 flex justify-end gap-2">
                      <button disabled={saving} onClick={() => setIsEditing(false)} className="px-4 py-2 border border-gray-300 dark:border-gray-600 rounded-lg text-gray-700 dark:text-gray-300 hover:bg-gray-50 dark:bg-gray-700 disabled:opacity-50">Cancel</button>
              ...
              <div className="bg-gray-100 dark:bg-gray-900 rounded-2xl shadow-sm p-6">
                <div className="flex items-center justify-between">
                  <div>
                    ...
                    <p className="text-lg font-bold text-gray-900 dark:text-gray-100 capitalize">{effectiveUser.role}</p>
                  </div>
