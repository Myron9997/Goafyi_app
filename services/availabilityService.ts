...
export class AvailabilityService {
  ...
  static async addBlockedDates(vendorId: string, dates: string[], reason?: string): Promise<void> {
    ...
    const payload = dates.map(d => ({ vendor_id: vendorId, date: d, reason }))
  ...
  static async removeBlockedDate(vendorId: string, date: string): Promise<void> {
    const { error } = await supabase
      .from('blocked_dates')
      .delete()
      .eq('vendor_id', vendorId)
  ...
  // Replace all blocked dates for a vendor in one go
  static async replaceBlockedDates(vendorId: string, dates: string[], reason?: string): Promise<void> {
    // Delete existing
