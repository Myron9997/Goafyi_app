...
export class RatingService {
  ...
  private static clearCache(vendorId: string): void {
    try {
      localStorage.removeItem(`${this.CACHE_PREFIX}${vendorId}`);
      localStorage.removeItem(`${this.STATS_CACHE_PREFIX}${vendorId}`);
    } catch {
      // ignore errors
    }
  }
  ...
  static async getVendorRatingStats(vendorId: string): Promise<RatingStats | null> {
    try {
      ...
      const cached = this.getCache<RatingStats>(cacheKey, this.CACHE_TTL);
      if (cached) {
        return cached;
      }
  ...
  static async getUserRating(vendorId: string, userId: string): Promise<VendorRating | null> {
    try {
      ...
      const cached = this.getCache<VendorRating | null>(cacheKey, this.CACHE_TTL);
      if (cached !== null) {
        return cached;
      }
  ...
  static async updateRating(
    ...
    try {
      ...
      // Clear cache for this vendor since rating has changed
      this.clearCache(data.vendor_id);
  ...
  static async deleteRating(ratingId: string): Promise<boolean> {
    try {
      ...
      // Clear cache for this vendor since rating has changed
      if (rating) {
        this.clearCache(rating.vendor_id);
      }
  ...
  static clearAllCache(): void {
    try {
      ...
      keys.forEach(key => {
        if (key.startsWith(this.CACHE_PREFIX) || key.startsWith(this.STATS_CACHE_PREFIX)) {
          localStorage.removeItem(key);
        }
      });
    } catch {
